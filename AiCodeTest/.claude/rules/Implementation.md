...
1. Plan 단계에서 수립한 설계를 바탕으로 프로젝트 규칙에 맞게 코드를 구현한다.
2. 예외 처리 및 응답 규격 준수: 사전에 정의된 ErrorCode.java와 GlobalExceptionHandler, 그리고 ApiResponse.java 규격을 반드시 준수하여 백엔드 구현을 진행하도록 명시합니다.

3. 병렬 개발 구조 허용: API 스펙과 Mock 데이터가 2. 계획 단계에서 확정되었다면, "Domain/Mapper/Service (BE) 구현"과 "프론트엔드 UI/Store (FE) 구현"이 동시에 진행될 수 있음을 명시하면 속도를 높일 수 있습니다.

4. 보안 및 권한 로직 적용: 인증/인가(Auth 모듈 연동), 토큰 검증, 전역 라우터 가드(Nuxt Middleware) 적용 로직을 구현 단계에 포함합니다.
## 백엔드 구현 순서
