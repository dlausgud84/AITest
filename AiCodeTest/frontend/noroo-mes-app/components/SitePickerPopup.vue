<template>
  ...
  <Teleport to="body">
    <div class="picker-overlay" @click.self="$emit('close')">
      <div class="picker-dialog" role="dialog" aria-modal="true" aria-label="사이트 선택">

        <!-- 헤더 -->
        <div class="picker-header">
          <span class="picker-title">사이트 선택</span>
          ...
          <table v-else class="picker-table">
            ...
            <tbody>
              <tr
                ...
                :key="item.siteId"
                class="picker-row"
                :class="{ 'picker-row--selected': selected === item.siteId }"
                @click="select(item)"
                @dblclick="confirm(item)"
              >
                <td class="cell-id">{{ item.siteId }}</td>
                <td>{{ item.siteName }}</td>
              </tr>
            </tbody>
          </table>
...
import type { SiteItem } from '~/composables/useSiteAPI'

const emit = defineEmits<{
  select: [siteId: string, siteName: string]
  close:  []
}>()
...
const filtered = computed(() => {
  ...
  if (!kw) return siteList.value
  return siteList.value.filter(
    item =>
      item.siteId.toLowerCase().includes(kw) ||
...
function select(item: SiteItem) {
  selected.value = item.siteId
}

function confirm(item: SiteItem) {
  emit('select', item.siteId, item.siteName)
}

function confirmSelected() {
  const item = siteList.value.find(i => i.siteId === selected.value)
  if (item) emit('select', item.siteId, item.siteName)
}
</script>
